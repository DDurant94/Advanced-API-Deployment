...
from services.productService import save,find_all
...
def mock_product_data():
  ...
  mock_product.description = faker.text(max_nb_chars=80)
  return mock_product
...
def mock_products_data():
  ...
  mock_product1.description = faker.text(max_nb_chars=80)
  ...
  mock_product2.description = faker.text(max_nb_chars=80)
...
# Testing Services 
  ...
  def test_save_exception(self, mock_session):
      ...
      product_data = {'name': 'Laptop', 'price': 1000, 'quantity': 10,'description': "High end Laptop"}
  ...
  def test_find_all_exception(self, mock_paginate):
      mock_paginate.side_effect = Exception("Commit failed")
